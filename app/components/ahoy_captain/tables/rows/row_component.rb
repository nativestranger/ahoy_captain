    ...
    module Rows
      class RowComponent < ViewComponent::Base
        ...
        def progress_bar(value, max, label)
          items = []
          items << view_context.content_tag(:progress, "", class: "progress-primary bg-base-100 h-8 w-full max-w-2xl", value: value, max: max)
          items << view_context.content_tag(:span, class: "text-elipsis overflow-hidden absolute left-4 bottom-3 h-8 text-primary-content") do
            label
          end
